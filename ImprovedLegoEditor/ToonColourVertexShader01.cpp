...
*  ToonColourVertexShader01.cpp
*  Contains the function definitions for the ToonColourVertexShader01 class.
*/

#include "ToonColourVertexShader01.h"

// The vertex input layout expected by this vertex shader
const D3D11_INPUT_ELEMENT_DESC ToonColourVertexShader01::m_sInputLayoutDescription[] = 
{
	// Vertex data
...
ToonColourVertexShader01::ToonColourVertexShader01( void ) : VertexShader(), m_pCbPerFrameBuffer( nullptr)
{
...
//--------------------------------------------------------------------------------------
HRESULT ToonColourVertexShader01::Initialise( ID3D11Device* pDevice )
{
	HRESULT hr;

	// Create the shader

	hr = pDevice -> CreateVertexShader( g_toonColourVertexShader01, sizeof( g_toonColourVertexShader01 ), nullptr, &m_pVertexShader );
	if( FAILED ( hr ) )
	{
		return hr;
	}
	...
	hr = pDevice -> CreateInputLayout( m_sInputLayoutDescription, numElements, g_toonColourVertexShader01, sizeof( g_toonColourVertexShader01 ), &m_pInputLayout );
	if( FAILED ( hr ) )
	{
		return hr;
	}
...
//--------------------------------------------------------------------------------------
HRESULT ToonColourVertexShader01::Cleanup( void )
{
...
//--------------------------------------------------------------------------------------
HRESULT ToonColourVertexShader01::UpdatePerSceneData( ID3D11DeviceContext* pContext, const PerSceneData& perSceneData)
{
...
//--------------------------------------------------------------------------------------
HRESULT ToonColourVertexShader01::UpdatePerFrameData( ID3D11DeviceContext* pContext, const PerFrameData& perFrameData)
{
	HRESULT hr;
...
//--------------------------------------------------------------------------------------
HRESULT ToonColourVertexShader01::UpdatePerObjectData( ID3D11DeviceContext* pContext, const PerObjectData& perObjectData)
{
...
//--------------------------------------------------------------------------------------
HRESULT ToonColourVertexShader01::UpdateTextureResource( int index, ID3D11DeviceContext* pContext, ID3D11ShaderResourceView* pTexture, ID3D11SamplerState* pSamplerState )
{
	// Not used in this shader
	return E_FAIL;
}
