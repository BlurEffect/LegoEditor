...
*  CreateMapsVertexShaderToon06.cpp
*  Contains the function definitions for the CreateMapsVertexShaderToon06 class.
*/

#include "CreateMapsVertexShaderToon06.h"

// The vertex input layout expected by this vertex shader
const D3D11_INPUT_ELEMENT_DESC CreateMapsVertexShaderToon06::m_sInputLayoutDescription[] = 
{
	// Vertex data
...
CreateMapsVertexShaderToon06::CreateMapsVertexShaderToon06( void ) : VertexShader(), m_pCbPerFrameBuffer( nullptr)
{
	// Initialise constant buffer data

	XMStoreFloat4x4( &m_cbPerFrame.m_viewProjection, XMMatrixIdentity() );
	XMStoreFloat4x4( &m_cbPerFrame.m_viewMatrix, XMMatrixIdentity() );
}

//--------------------------------------------------------------------------------------
// Initialise the shader's member variables.
//--------------------------------------------------------------------------------------
HRESULT CreateMapsVertexShaderToon06::Initialise( ID3D11Device* pDevice )
{
	HRESULT hr;

	// Create the shader

	hr = pDevice -> CreateVertexShader( g_createMapsVertexShader06, sizeof( g_createMapsVertexShader06 ), nullptr, &m_pVertexShader );
	if( FAILED ( hr ) )
	{
		return hr;
	}
	...
	hr = pDevice -> CreateInputLayout( m_sInputLayoutDescription, numElements, g_createMapsVertexShader06, sizeof( g_createMapsVertexShader06 ), &m_pInputLayout );
	if( FAILED ( hr ) )
	{
		return hr;
	}
...
//--------------------------------------------------------------------------------------
HRESULT CreateMapsVertexShaderToon06::Cleanup( void )
{
...
//--------------------------------------------------------------------------------------
HRESULT CreateMapsVertexShaderToon06::UpdatePerSceneData( ID3D11DeviceContext* pContext, const PerSceneData& perSceneData)
{
...
//--------------------------------------------------------------------------------------
HRESULT CreateMapsVertexShaderToon06::UpdatePerFrameData( ID3D11DeviceContext* pContext, const PerFrameData& perFrameData)
{
	HRESULT hr;

	m_cbPerFrame.m_viewProjection = perFrameData.m_viewProjection;
	m_cbPerFrame.m_viewMatrix	  = perFrameData.m_viewMatrix;
...
//--------------------------------------------------------------------------------------
HRESULT CreateMapsVertexShaderToon06::UpdatePerObjectData( ID3D11DeviceContext* pContext, const PerObjectData& perObjectData)
{
...
//--------------------------------------------------------------------------------------
HRESULT CreateMapsVertexShaderToon06::UpdateTextureResource( int index, ID3D11DeviceContext* pContext, ID3D11ShaderResourceView* pTexture, ID3D11SamplerState* pSamplerState )
{
	// Not used in this shader
	return E_FAIL;
}
