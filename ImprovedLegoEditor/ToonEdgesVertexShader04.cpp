...
*  ToonEdgesVertexShader04.cpp
*  Contains the function definitions for the ToonEdgesVertexShader04 class.
*/

#include "ToonEdgesVertexShader04.h"

// The vertex input layout expected by this vertex shader
const D3D11_INPUT_ELEMENT_DESC ToonEdgesVertexShader04::m_sInputLayoutDescription[] = 
{
	// Vertex data
...
ToonEdgesVertexShader04::ToonEdgesVertexShader04( void ) : VertexShader(), m_pCbPerSceneBuffer( nullptr ), m_pCbPerFrameBuffer( nullptr)
{
	// Initialise constant buffer data

	m_cbPerScene.m_moveBias = 0.0f;
	m_cbPerScene.m_padding   = XMFLOAT3( 0.0f, 0.0f, 0.0f );

	XMStoreFloat4x4( &m_cbPerFrame.m_viewProjection, XMMatrixIdentity() );
	
	m_cbPerFrame.m_eyePosition = XMFLOAT3( 0.0f, 0.0f, 0.0f );
	m_cbPerFrame.m_padding	   = 0.0f;
}

//--------------------------------------------------------------------------------------
// Initialise the shader's member variables.
//--------------------------------------------------------------------------------------
HRESULT ToonEdgesVertexShader04::Initialise( ID3D11Device* pDevice )
{
	HRESULT hr;

	// Create the shader

	hr = pDevice -> CreateVertexShader( g_toonEdgesVertexShader04, sizeof( g_toonEdgesVertexShader04 ), nullptr, &m_pVertexShader );
	if( FAILED ( hr ) )
	{
		return hr;
	}
	...
	hr = pDevice -> CreateInputLayout( m_sInputLayoutDescription, numElements, g_toonEdgesVertexShader04, sizeof( g_toonEdgesVertexShader04 ), &m_pInputLayout );
	if( FAILED ( hr ) )
	{
		return hr;
	}
...
//--------------------------------------------------------------------------------------
HRESULT ToonEdgesVertexShader04::Cleanup( void )
{
...
//--------------------------------------------------------------------------------------
HRESULT ToonEdgesVertexShader04::UpdatePerSceneData( ID3D11DeviceContext* pContext, const PerSceneData& perSceneData)
{
	HRESULT hr;

	m_cbPerScene.m_moveBias = perSceneData.m_moveBias;
...
//--------------------------------------------------------------------------------------
HRESULT ToonEdgesVertexShader04::UpdatePerFrameData( ID3D11DeviceContext* pContext, const PerFrameData& perFrameData)
{
	HRESULT hr;

	m_cbPerFrame.m_viewProjection = perFrameData.m_viewProjection;
	m_cbPerFrame.m_eyePosition = perFrameData.m_viewPosition;
...
//--------------------------------------------------------------------------------------
HRESULT ToonEdgesVertexShader04::UpdatePerObjectData( ID3D11DeviceContext* pContext, const PerObjectData& perObjectData)
{
...
//--------------------------------------------------------------------------------------
HRESULT ToonEdgesVertexShader04::UpdateTextureResource( int index, ID3D11DeviceContext* pContext, ID3D11ShaderResourceView* pTexture, ID3D11SamplerState* pSamplerState )
{
	// Not used in this shader
	return E_FAIL;
}
