...
*  LightedVertexShader.cpp
*  Contains the function definitions for the LightedVertexShader class.
*/

#include "LightedVertexShader.h"

// The vertex input layout expected by this vertex shader
const D3D11_INPUT_ELEMENT_DESC LightedVertexShader::m_sInputLayoutDescription[] = 
{
	// Vertex data
...
LightedVertexShader::LightedVertexShader( void ) : VertexShader(), m_pCbPerFrameBuffer( nullptr)
{
...
//--------------------------------------------------------------------------------------
HRESULT LightedVertexShader::Initialise( ID3D11Device* pDevice )
{
	HRESULT hr;

	// Create the shader

	hr = pDevice -> CreateVertexShader( g_lightedVertexShader, sizeof( g_lightedVertexShader ), nullptr, &m_pVertexShader );
	if( FAILED ( hr ) )
	{
		return hr;
	}
	...
	hr = pDevice -> CreateInputLayout( m_sInputLayoutDescription, numElements, g_lightedVertexShader, sizeof( g_lightedVertexShader ), &m_pInputLayout );
	if( FAILED ( hr ) )
	{
		return hr;
	}
...
//--------------------------------------------------------------------------------------
HRESULT LightedVertexShader::Cleanup( void )
{
...
//--------------------------------------------------------------------------------------
HRESULT LightedVertexShader::UpdatePerSceneData( ID3D11DeviceContext* pContext, const PerSceneData& perSceneData)
{
...
//--------------------------------------------------------------------------------------
HRESULT LightedVertexShader::UpdatePerFrameData( ID3D11DeviceContext* pContext, const PerFrameData& perFrameData)
{
	HRESULT hr;
...
//--------------------------------------------------------------------------------------
HRESULT LightedVertexShader::UpdatePerObjectData( ID3D11DeviceContext* pContext, const PerObjectData& perObjectData)
{
...
//--------------------------------------------------------------------------------------
HRESULT LightedVertexShader::UpdateTextureResource( int index, ID3D11DeviceContext* pContext, ID3D11ShaderResourceView* pTexture, ID3D11SamplerState* pSamplerState )
{
	// Not used in this shader
	return E_FAIL;
}
