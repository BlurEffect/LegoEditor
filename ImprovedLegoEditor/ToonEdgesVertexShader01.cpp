...
*  ToonEdgesVertexShader01.cpp
*  Contains the function definitions for the ToonEdgesVertexShader01 class.
*/

#include "ToonEdgesVertexShader01.h"

// The vertex input layout expected by this vertex shader
const D3D11_INPUT_ELEMENT_DESC ToonEdgesVertexShader01::m_sInputLayoutDescription[] = 
{
	// Vertex data
...
ToonEdgesVertexShader01::ToonEdgesVertexShader01( void ) : VertexShader(), m_pCbPerSceneBuffer( nullptr ), m_pCbPerFrameBuffer( nullptr)
{
...
//--------------------------------------------------------------------------------------
HRESULT ToonEdgesVertexShader01::Initialise( ID3D11Device* pDevice )
{
	HRESULT hr;

	// Create the shader

	hr = pDevice -> CreateVertexShader( g_toonEdgesVertexShader01, sizeof( g_toonEdgesVertexShader01 ), nullptr, &m_pVertexShader );
	if( FAILED ( hr ) )
	{
		return hr;
	}
	...
	hr = pDevice -> CreateInputLayout( m_sInputLayoutDescription, numElements, g_toonEdgesVertexShader01, sizeof( g_toonEdgesVertexShader01 ), &m_pInputLayout );
	if( FAILED ( hr ) )
	{
		return hr;
	}
...
//--------------------------------------------------------------------------------------
HRESULT ToonEdgesVertexShader01::Cleanup( void )
{
...
//--------------------------------------------------------------------------------------
HRESULT ToonEdgesVertexShader01::UpdatePerSceneData( ID3D11DeviceContext* pContext, const PerSceneData& perSceneData)
{
	HRESULT hr;

	m_cbPerScene.m_vertexOffset = perSceneData.m_vertexOffset01;
...
//--------------------------------------------------------------------------------------
HRESULT ToonEdgesVertexShader01::UpdatePerFrameData( ID3D11DeviceContext* pContext, const PerFrameData& perFrameData)
{
	HRESULT hr;
...
//--------------------------------------------------------------------------------------
HRESULT ToonEdgesVertexShader01::UpdatePerObjectData( ID3D11DeviceContext* pContext, const PerObjectData& perObjectData)
{
...
//--------------------------------------------------------------------------------------
HRESULT ToonEdgesVertexShader01::UpdateTextureResource( int index, ID3D11DeviceContext* pContext, ID3D11ShaderResourceView* pTexture, ID3D11SamplerState* pSamplerState )
{
	// Not used in this shader
	return E_FAIL;
}
