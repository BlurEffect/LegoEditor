...
*  ToonEdgesVertexShader05.cpp
*  Contains the function definitions for the ToonEdgesVertexShader05 class.
*/

#include "ToonEdgesVertexShader05.h"

// The vertex input layout expected by this vertex shader
const D3D11_INPUT_ELEMENT_DESC ToonEdgesVertexShader05::m_sInputLayoutDescription[] = 
{
	// Vertex data
...
ToonEdgesVertexShader05::ToonEdgesVertexShader05( void ) : VertexShader(), m_pCbPerSceneBuffer( nullptr ), m_pCbPerFrameBuffer( nullptr )
{
	// Initialise constant buffer data

	m_cbPerScene.m_outlineWidth = 0.0f;
...
//--------------------------------------------------------------------------------------
HRESULT ToonEdgesVertexShader05::Initialise( ID3D11Device* pDevice )
{
	HRESULT hr;

	// Create the shader

	hr = pDevice -> CreateVertexShader( g_toonEdgesVertexShader05, sizeof( g_toonEdgesVertexShader05 ), nullptr, &m_pVertexShader );
	if( FAILED ( hr ) )
	{
		return hr;
	}
	...
	hr = pDevice -> CreateInputLayout( m_sInputLayoutDescription, numElements, g_toonEdgesVertexShader05, sizeof( g_toonEdgesVertexShader05 ), &m_pInputLayout );
	if( FAILED ( hr ) )
	{
		return hr;
	}
...
//--------------------------------------------------------------------------------------
HRESULT ToonEdgesVertexShader05::Cleanup( void )
{
...
//--------------------------------------------------------------------------------------
HRESULT ToonEdgesVertexShader05::UpdatePerSceneData( ID3D11DeviceContext* pContext, const PerSceneData& perSceneData)
{
	HRESULT hr;

	m_cbPerScene.m_outlineWidth = perSceneData.m_outlineWidth;
...
//--------------------------------------------------------------------------------------
HRESULT ToonEdgesVertexShader05::UpdatePerFrameData( ID3D11DeviceContext* pContext, const PerFrameData& perFrameData)
{
	HRESULT hr;
...
//--------------------------------------------------------------------------------------
HRESULT ToonEdgesVertexShader05::UpdatePerObjectData( ID3D11DeviceContext* pContext, const PerObjectData& perObjectData)
{
...
//--------------------------------------------------------------------------------------
HRESULT ToonEdgesVertexShader05::UpdateTextureResource( int index, ID3D11DeviceContext* pContext, ID3D11ShaderResourceView* pTexture, ID3D11SamplerState* pSamplerState )
{
	// Not used in this shader
	return E_FAIL;
}
