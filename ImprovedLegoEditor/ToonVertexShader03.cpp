...
*  ToonVertexShader03.cpp
*  Contains the function definitions for the ToonVertexShader03 class.
*/

#include "ToonVertexShader03.h"

// The vertex input layout expected by this vertex shader
const D3D11_INPUT_ELEMENT_DESC ToonVertexShader03::m_sInputLayoutDescription[] = 
{
	// Vertex data
...
ToonVertexShader03::ToonVertexShader03( void ) : VertexShader(), m_pCbPerFrameBuffer( nullptr)
{
	// Initialise constant buffer data

	XMStoreFloat4x4( &m_cbPerFrame.m_viewProjection, XMMatrixIdentity() );
	m_cbPerFrame.m_viewPosition = XMFLOAT3( 0.0f, 0.0f, 0.0f );	
...
//--------------------------------------------------------------------------------------
HRESULT ToonVertexShader03::Initialise( ID3D11Device* pDevice )
{
	HRESULT hr;

	// Create the shader

	hr = pDevice -> CreateVertexShader( g_toonVertexShader03, sizeof( g_toonVertexShader03 ), nullptr, &m_pVertexShader );
	if( FAILED ( hr ) )
	{
		return hr;
	}
	...
	hr = pDevice -> CreateInputLayout( m_sInputLayoutDescription, numElements, g_toonVertexShader03, sizeof( g_toonVertexShader03 ), &m_pInputLayout );
	if( FAILED ( hr ) )
	{
		return hr;
	}
...
//--------------------------------------------------------------------------------------
HRESULT ToonVertexShader03::Cleanup( void )
{
...
//--------------------------------------------------------------------------------------
HRESULT ToonVertexShader03::UpdatePerSceneData( ID3D11DeviceContext* pContext, const PerSceneData& perSceneData)
{
...
//--------------------------------------------------------------------------------------
HRESULT ToonVertexShader03::UpdatePerFrameData( ID3D11DeviceContext* pContext, const PerFrameData& perFrameData)
{
	HRESULT hr;
	
	m_cbPerFrame.m_viewProjection = perFrameData.m_viewProjection;
	m_cbPerFrame.m_viewPosition   = perFrameData.m_viewPosition;
...
//--------------------------------------------------------------------------------------
HRESULT ToonVertexShader03::UpdatePerObjectData( ID3D11DeviceContext* pContext, const PerObjectData& perObjectData)
{
...
//--------------------------------------------------------------------------------------
HRESULT ToonVertexShader03::UpdateTextureResource( int index, ID3D11DeviceContext* pContext, ID3D11ShaderResourceView* pTexture, ID3D11SamplerState* pSamplerState )
{
	// Not used in this shader
	return E_FAIL;
}
